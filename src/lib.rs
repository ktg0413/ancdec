    ...
    impl Encode<'_, Postgres> for AncDec {
        fn encode_by_ref(&self, buf: &mut PgArgumentBuffer) -> Result<IsNull, BoxDynError> {
            ...
            if self.scale > 0 && frac_val > 0 {
                ...
                let padded_scale = ((self.scale + 3) / 4) * 4;
                let pad_mult = super::pow10((padded_scale - self.scale) as u8);
                frac_val *= pad_mult as u64;
            ...
            while digits.len() > 0 && digits.last() == Some(&0) {
                digits.pop();
            }
            ...
            while digits.len() > 0 && digits.first() == Some(&0) {
                digits.remove(0);
            }
                ...
                PgValueFormat::Binary => {
                    ...
                    if dscale > 0 {
                        ...
                        for i in frac_start..digits.len() {
                            frac_str.push_str(&std::format!("{:04}", digits[i]));
                        }
